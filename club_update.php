...
if(!empty($klub)){
    ...
    if($uploadOk == 1) {
        // uporabnik je naložil logotip
        if (move_uploaded_file($_FILES["grb"]["tmp_name"], $grb)) {
        $query = "UPDATE klubi SET klub=?, opis=?, grb=? WHERE id_klubi=?";

        $stmt = $pdo->prepare($query);
        $stmt->execute([$klub,$opis,$grb,$id]);

        header("Location: club.php?id=$id");
        die();
    } else{
        header("Location: club_edit.php?id=$id");
        die();
    }
    }
    else{
        //uorabnik ni naložil logotipa
        $query = "UPDATE klubi SET klub=?, opis=? WHERE id_klubi=?";

        $stmt = $pdo->prepare($query);
        $stmt->execute([$klub,$opis,$id]);
